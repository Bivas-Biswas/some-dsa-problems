...
class SegmentTree {
public:
    vector<int> t; // stores count of even numbers
    ...
    // Build tree: store 1 if arr[l] is even, else 0
    ...
    // Query count of evens in range [lq, rq]
...
int main() {
    ...
    while (Q--) {
        ...
        cin >> type >> x >> y;
        if (type == 0) {
            // Update: set A[x] = y (x is 1-indexed in problem statement)
            segt.update(1, 0, N - 1, x - 1, y);
        } else {
            // Query: count evens in [l, r] (1-indexed)
